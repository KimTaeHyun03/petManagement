...
> ⚠ **상위 폴더(`D:\capstone\AGENTS.md`)는 참조하지 말 것.** 그 파일은 다른 프로젝트(데이트 코스 추천 앱) 기준이라 이 저장소와 도메인이 다르다. 이 저장소의 명세·관습은 본 파일과 `PLAN.md` / `function.md` / `workflow.md` / `RESEARCH.md`만으로 결정한다.
