...
- [X] **회원가입 + 로그인 + 펫 등록 commit** — 사용자 검토 후 진행 (현재 검토 단계, working tree에 staged)
- [X] **DB 엔진 결정 (PostgreSQL vs MySQL)** — 김찬영과 협의. 결정 후 `PLAN.md` 10절·`CLAUDE.md`·`server/migrations/` 갱신
  - 현 코드는 **PostgreSQL 가정** (pgcrypto, `$1` placeholder, jsonb). MySQL로 결정되면 마이그레이션 + repo 쿼리 재작성 필요.
- [X] **AWS RDS 인스턴스 셋업** — 담당: 김찬영. endpoint·DB명·계정 정보 공유 필요
...
- [X] `npm run migrate` 로 `001_create_users.sql` + `002_create_pets.sql` 적용
- [X] 회원가입 엔드포인트 실제 호출 테스트 — 정상 1건 + 5가지 실패 케이스(중복/짧은 비번/Pwned/이메일 형식/Rate limit)
- [X] 로그인 엔드포인트 실제 호출 테스트 — 정상 / 잘못된 비번 / 미가입 이메일(타이밍 일정 확인) / Rate limit
